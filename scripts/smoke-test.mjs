...
assert.equal(DEFAULT_PROVIDERS.find((provider) => provider.id === 'gemini')?.model, 'gemini-3.6-flash', 'Gemini should default to 3.6 Flash');
const calc = await utilityAnswer('calculate 12 * (3 + 2)');
assert.equal(calc?.kind, 'utility');
assert.equal(calc?.answer, '12 * (3 + 2) = 60');
...
assert.equal(emptyModels.flat().length, 0, 'unconfigured providers must not make network calls');
const webFallback = await failoverChat(DEFAULT_PROVIDERS, [{ role: 'user', content: 'latest technology news' }]); assert.equal(webFallback.provider, 'web', 'no-key failover should switch to sourced web search'); assert.match(webFallback.answer, /Web lookup|No indexed web results|Internet lookup is unavailable/);
