...
const DEFAULT_IDENTITY: VerifiedIdentity = {
...
let cachedIdentity: VerifiedIdentity | null = null;

function locateIdentityDat(): string | null {
...
export function loadAndVerifyIdentity(): VerifiedIdentity {
  ...
  try {
    ...
    while (keystream.length < payload.length) {
      ...
      for (const b of block) keystream.push(b);
    }
...
export type IdentityIntent = 'creator' | 'model' | 'identity' | 'version' | 'codename';

export function detectIdentityIntent(query: string): IdentityIntent | null {
  const q = query.toLowerCase().trim().replace(/['"`?.,!]/g, '');

  // 1. Creator inquiries
  if (
    /^(who|whom)\s+(created|made|built|developed|designed|founded|programmed|coded|authored)\s+(you|jarvis|this ai|the ai)/i.test(q) ||
  ...
  // 2. Model inquiries
  if (
    /^(what|whats|which)\s+(is\s+)?(your\s+)?(model|ai model|base model|llm|neural network|architecture|engine)/i.test(q) ||
  ...
  if (
    /^(what|whats)\s+(is\s+)?(your\s+)?(ai\s+)?code\s*name/i.test(q) ||
  ...
  // 4. Version inquiries
  if (
    /^(what|whats)\s+(is\s+)?(your\s+)?(version|system version|architecture version|mark)/i.test(q) ||
...
export function getHardenedIdentityAnswer(intent: IdentityIntent, identity: VerifiedIdentity): string {
  switch (intent) {
    case 'creator':
...
export function getHardenedSystemDirective(identity: VerifiedIdentity): string {
  return [
...
export function sanitizeAIResponse(text: string, identity: VerifiedIdentity): string {
  if (!text) return text;
  let sanitized = text;
