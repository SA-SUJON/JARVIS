...
export async function webSearchAnswer(query: string): Promise<{ kind: 'search'; answer: string } | null> {
  ...
  try {
    ...
    const lines: string[] = [`Web lookup for “${cleanQuery}”.`];
    if (instantJson.AbstractText) lines.push(`${instantJson.Heading ? `${instantJson.Heading}: ` : ''}${instantJson.AbstractText}${instantJson.AbstractURL ? `\nSource: ${instantJson.AbstractURL}` : ''}`);
    hits.forEach((hit, index) => lines.push(`\n${index + 1}. ${hit.title}\n${hit.snippet || 'Open the source for the full result.'}\nSource: ${hit.url}`));
    if (lines.length === 1) return { kind: 'search', answer: `No indexed web results were returned for “${cleanQuery}”. Try a more specific query.` };
...
export async function listModels(provider: ProviderConfig): Promise<ModelInfo[]> {
  ...
  if (provider.id === 'cohere') {
    ...
    if (!res.ok) throw new Error(await responseError(res));
    const json = await res.json() as { models?: Array<{ name?: string; id?: string }> };
  ...
  if (provider.id === 'anthropic') {
    ...
    if (!res.ok) throw new Error(await responseError(res));
    const json = await res.json() as { data?: Array<{ id: string; display_name?: string; created_at?: string; max_input_tokens?: number }> };
...
export async function chat(provider: ProviderConfig, messages: ChatMessage[], model: string, signal?: AbortSignal): Promise<string> {
  if (!provider.key) throw new Error('API key not configured');
  if (provider.id === 'gemini') {
  ...
  if (provider.id === 'cohere') {
    const system = messages.find((m) => m.role === 'system')?.content; const userMessages = messages.filter((m) => m.role !== 'system');
...
export async function utilityAnswer(query: string): Promise<{ kind: 'utility' | 'search'; answer: string } | null> {
  ...
  if (/^(what(?:'s| is) |tell me )?(the )?time(?: in | at )?/i.test(text)) {
    ...
    try {
      ...
      if (res.ok) { const json = await res.json() as { datetime?: string; timezone?: string }; return { kind: 'utility', answer: `Local time in ${json.timezone || tz}: ${new Date(json.datetime || Date.now()).toLocaleString()}.` }; }
  ...
  if (/(weather|temperature|forecast)/i.test(text)) {
    ...
    try {
      ...
      if (res.ok) { const json = await res.json() as { current_condition?: Array<{ temp_C?: string; weatherDesc?: Array<{ value?: string }>; humidity?: string }> }; const c = json.current_condition?.[0]; return { kind: 'utility', answer: `Weather for ${city}: ${c?.weatherDesc?.[0]?.value || 'available'}, ${c?.temp_C || '--'}°C, humidity ${c?.humidity || '--'}%.` }; }
    } catch { /* fall through */ }
  }
  if (/^(calculate|compute|what is)\s+/i.test(text)) {
    ...
    if (result !== null) return { kind: 'utility', answer: `${expression} = ${result}` };
  }
...
export async function failoverChat(providers: ProviderConfig[], messages: ChatMessage[], preferred?: ProviderId): Promise<{ answer: string; provider: ProviderId | 'web'; attempts: string[] }> {
  ...
  if (!ordered.length) { const web = await webSearchAnswer(lastUserQuery); if (web) return { answer: web.answer, provider: 'web', attempts: ['No configured AI keys; switched to sourced web search.'] }; throw new Error('Configure at least one provider API key in SYSTEM > NEURAL LINK.'); }
  const attempts: string[] = [];
  for (const provider of ordered) {
    try {
      const defaults: Record<ProviderId, string> = { openai: 'gpt-4o-mini', gemini: 'gemini-3.6-flash', anthropic: 'claude-3-5-haiku-latest', deepseek: 'deepseek-chat', xai: 'grok-3-mini', qwen: 'qwen-plus', kimi: 'moonshot-v1-8k', minimax: 'MiniMax-Text-01', perplexity: 'sonar', manus: 'manus', groq: 'openai/gpt-oss-120b', cohere: 'command-a-03-2025', huggingface: 'meta-llama/Llama-3.1-8B-Instruct' }; const model = provider.model || defaults[provider.id];
      ...
      const status = Number(message.match(/^\d+/)?.[0] || 500); const quotaMessage = /quota|rate.?limit|token|credit|capacity|context.?length|exhaust|insufficient/i.test(message); if (!isQuotaLike(status) && !quotaMessage) throw error;
    }
  }
  const web = await webSearchAnswer(lastUserQuery);
  if (web) return { answer: web.answer, provider: 'web', attempts: [...attempts, 'All configured AI links exhausted; switched to sourced web search.'] };
