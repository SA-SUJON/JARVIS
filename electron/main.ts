...
import { createMark05Runtime } from './mark05Runtime.js';
import type { ChatMessage as CoreChatMessage } from '../core/contracts/types.js';
...
function resolvePythonExecutable(root: string) {
  ...
  if (existsSync(venvPy) && existsSync(path.join(venvSitePackages, 'selenium'))) return venvPy;
  return 'python';
}
...
function ensurePythonBridge() {
  ...
  pythonBridge.once('error', (error) => { pythonBridge = null; rejectPythonPending(error); mainWindow?.webContents.send('python:error', error.message); });
  pythonBridge.once('close', (code, signal) => { const error = new Error(`Python capability bridge exited (code=${code ?? 'unknown'}, signal=${signal ?? 'none'})`); pythonBridge = null; pythonBridgeBuffer = ''; rejectPythonPending(error); mainWindow?.webContents.send('python:stopped', { code, signal }); });
  return pythonBridge;
}
...
function rawPythonRequest(op: string, payload: Record<string, any>, timeoutMs = 120000) {
  ...
  return new Promise<any>((resolve, reject) => {
    const timer = setTimeout(() => { pythonPending.delete(id); reject(new Error(`Python capability timeout: ${op}`)); }, timeoutMs);
    pythonPending.set(id, { resolve, reject, timer });
    try { child.stdin.write(`${JSON.stringify({ id, op, ...(op === 'configure' ? { config: payload } : { payload }) })}\n`); }
...
async function imageDataUrls(files: string[]) {
  return await Promise.all(files.slice(0, 4).map(async (file) => {
...
async function readSettings(): Promise<PersistedSettings> {
  ...
  try {
    ...
    const decryptedGeminiKeys: string[] = (raw.encryptedGeminiKeys || []).map((k) => decryptSecret(k)).filter((k): k is string => Boolean(k));
    ...
    return { providers: DEFAULT_PROVIDERS.map((p) => ({ ...p })), geminiKeys: defaults.geminiKeys || [], voice: defaults.voice, voiceProfile: 'natural', language: defaults.language, wakeWord: true, voiceEnabled: true, assistantName: defaults.assistantName, userName: defaults.userName, legacyKeys: defaults.legacyKeys };
  }
}

async function writeSettings(partial: Partial<PersistedSettings>) {
  const current = await readSettings();
  const providers = (partial.providers ?? current.providers).map((provider) => ({ ...provider }));
  const encryptedKeys: Record<string, string> = {};
  const encryptedLegacyKeys: Record<string, string> = {};
  for (const provider of providers) {
    if (provider.id === 'gemini') continue;
    const key = encryptSecret(provider.key);
    if (key) encryptedKeys[provider.id] = key;
    delete provider.key;
  }
  const gemini = providers.find((provider) => provider.id === 'gemini');
  const geminiRaw = partial.geminiKeys ?? gemini?.keys ?? (gemini?.key ? [gemini.key] : current.geminiKeys || []);
  const encryptedGeminiKeys = geminiRaw.map((key) => encryptSecret(key)).filter((key): key is string => Boolean(key));
  if (gemini) { const first = geminiRaw[0]; delete gemini.key; delete gemini.keys; if (first && !encryptedGeminiKeys.length) throw new Error('OS secure storage unavailable; provider credentials were not persisted.'); }
  for (const [name, value] of Object.entries(partial.legacyKeys ?? current.legacyKeys)) {
    const encrypted = encryptSecret(value);
    if (encrypted) encryptedLegacyKeys[name] = encrypted;
  }
  const persisted = {
    providers,
    encryptedKeys,
    ...
    voiceProfile: partial.voiceProfile ?? current.voiceProfile,
    language: partial.language ?? current.language,
    wakeWord: partial.wakeWord ?? current.wakeWord,
    voiceEnabled: partial.voiceEnabled ?? current.voiceEnabled,
    assistantName: partial.assistantName ?? current.assistantName,
...
async function handleLegacyAssistantQuery(input: { query: string; providers: ProviderConfig[]; preferred?: ProviderId; history?: ChatMessage[] }) {
  const verifiedIdentity = loadAndVerifyIdentity();
  const identityIntent = detectIdentityIntent(input.query);
  if (identityIntent) return { kind: 'ai', answer: getHardenedIdentityAnswer(identityIntent, verifiedIdentity), provider: 'identity', attempts: [] };
  ...
  try { decisions = (await callPython('classify', { prompt: input.query }, identity, 45000)).decisions || []; } catch { decisions = deterministicDecisions(input.query); }
  for (const decision of decisions) {
    if (/^generate image /i.test(decision)) {
      try {
        const generated = await callPython('image', { prompt: decision.replace(/^generate image /i, '') }, identity, 180000);
        const images = await imageDataUrls(generated.files || []);
        return { kind: 'ai', answer: generated.answer || 'Image generation completed.', provider: 'jarvisai-python', attempts: [], images };
      } catch { /* continue to normal chat fallback */ }
    }
  }
...
function buildMark05CoreHistory(history: ChatMessage[] | undefined): CoreChatMessage[] | undefined {
  return history?.map((message) => ({ role: message.role, content: message.content }));
}

async function handleMark05AssistantQuery(input: { query: string; providers: ProviderConfig[]; preferred?: ProviderId; history?: ChatMessage[] }) {
  const settings = await readSettings();
  const runtime = createMark05Runtime({ providers: input.providers?.length ? input.providers : settings.providers });
  const result = await runtime.execute({
    input: input.query,
    history: buildMark05CoreHistory(input.history),
    preferredProvider: input.preferred,
  ...
  if (result.status === 'awaiting_approval') {
    return {
      kind: 'policy',
      answer: result.error || 'This action requires explicit approval before I can continue.',
      provider: 'mark05-policy',
      attempts: [],
  ...
  if (result.status === 'failed') {
    // Preserve deterministic local utility and legacy identity responses while MARK_05 matures.
    const verifiedIdentity = loadAndVerifyIdentity();
    const identityIntent = detectIdentityIntent(input.query);
    if (identityIntent) return { kind: 'ai', answer: getHardenedIdentityAnswer(identityIntent, verifiedIdentity), provider: 'identity', attempts: [] };
    const utility = await utilityAnswer(input.query, settings);
    if (utility) return { ...utility, provider: 'utility', attempts: [] };
    return { kind: 'ai', answer: result.error || 'MARK_05 runtime failed to execute the request.', provider: 'mark05', attempts: [] };
  }

  if (!result.response) return { kind: 'ai', answer: 'MARK_05 completed the request without a provider response.', provider: 'mark05', attempts: [] };
  const verifiedIdentity = loadAndVerifyIdentity();
  return {
    kind: 'ai',
    answer: sanitizeAIResponse(result.response.text, verifiedIdentity),
    provider: result.providerId || 'mark05',
    attempts: [],
    requestId: result.requestId,
    model: result.model,
  };
...
function registerIpcHandlers() {
  ...
  ipcMain.handle('face-profiles:save', async (_event, input) => { const next = normalizeFaceProfile(input); const profiles = await readFaceProfiles(); const withoutExisting = profiles.filter((profile) => profile.id !== next.id); await writeFaceProfiles([...withoutExisting, next]); return next; });
  ipcMain.handle('face-profiles:delete', async (_event, id: string) => { const profiles = await readFaceProfiles(); await writeFaceProfiles(profiles.filter((profile) => profile.id !== String(id))); return true; });
  ...
  ipcMain.handle('assistant:query', async (_event, input: { query: string; providers: ProviderConfig[]; preferred?: ProviderId; history?: ChatMessage[] }) => {
    try {
      return await handleMark05AssistantQuery(input);
    } catch (error) {
      try {
        return await handleLegacyAssistantQuery(input);
      } catch (legacyError) {
        return { kind: 'ai', answer: legacyError instanceof Error ? legacyError.message : String(legacyError), provider: 'failover', attempts: [] };
      }
    }
  });
  ...
  ipcMain.handle('stt:start', async (_event, language = 'en') => {
    ...
    if (!existsSync(script)) return { available: false, running: false, reason: `Speech recognition runtime not found at ${script}` };
    try {
      const executable = resolvePythonExecutable(pythonRoot);
      voiceListener = spawn(executable, ['-u', path.join('Backend', 'SpeechToText.py'), language], { cwd: pythonRoot, windowsHide: true, env: { ...process.env, PYTHONIOENCODING: 'utf-8' } });
      voiceListener.stdout.setEncoding('utf8');
      voiceListener.stdout.on('data', (chunk) => { const text = String(chunk); const match = text.match(/(?:transcript|text)\s*[:=]\s*(.+)/i); if (match?.[1]?.trim()) mainWindow?.webContents.send('stt:transcript', { text: match[1].trim() }); });
      voiceListener.stderr.setEncoding('utf8');
      voiceListener.stderr.on('data', (chunk) => { const detail = String(chunk).trim(); if (detail) { voiceListenerLastError = detail; mainWindow?.webContents.send('stt:error', detail); } });
      voiceListener.once('error', (error) => { voiceListener = null; mainWindow?.webContents.send('stt:error', `Python STT launch failed: ${error.message}`); });
      voiceListener.once('close', () => { voiceListener = null; if (!sttStopping && voiceListenerLastError) mainWindow?.webContents.send('stt:error', voiceListenerLastError); sttStopping = false; });
      ...
      voiceListener = null;
      return { available: false, running: false, reason: error instanceof Error ? error.message : String(error) };
    }
  });
  ...
  ipcMain.handle('voice:speak', async (_event, input: { text: string; voice: string; language?: string; voiceProfile?: VoiceProfile }) => {
    ...
    try {
      ...
      const output = audioPath('mp3');
      await tts.ttsPromise(speechText, output);
      const data = await fs.readFile(output);
      return { source: 'edge-tts', file: output, dataUrl: `data:audio/mpeg;base64,${data.toString('base64')}` };
    } catch (onlineError) {
      try {
        const settings = await readSettings();
        ...
        return { source: 'unavailable', error: `${onlineError instanceof Error ? onlineError.message : onlineError}; ${pythonError instanceof Error ? pythonError.message : pythonError}` };
      }
    }
  });
...
app.on('window-all-closed', () => {
  ...
  if (voiceListener) { sttStopping = true; voiceListener.kill(); }
  if (pythonBridge) { pythonBridge.kill(); pythonBridge = null; rejectPythonPending(new Error('Python capability bridge stopped with the application')); }
