...
const systemInformation: Record<string, (...args: any[]) => Promise<any>> | null = (() => { try { return require('systeminformation') as Record<string, (...args: any[]) => Promise<any>>; } catch { return null; } })();

function exec(command: string, args: string[], timeout = 8000): Promise<{ stdout: string; stderr: string; code: number }> {
  return new Promise((resolve) => {
    execFile(command, args, { timeout, windowsHide: true, maxBuffer: 1024 * 1024 }, (error, stdout, stderr) => resolve({ stdout: String(stdout || ''), stderr: String(stderr || ''), code: error ? (typeof (error as NodeJS.ErrnoException).code === 'number' ? Number((error as NodeJS.ErrnoException).code) : 1) : 0 }));
  });
}

async function siCall<T>(method: string, fallback: T): Promise<T> {
  try { return systemInformation?.[method] ? await systemInformation[method]() as T : fallback; } catch { return fallback; }
}

async function getWindowsNativeTelemetry() {
  if (process.platform !== 'win32') return { available: false, temperature: null, battery: null, cpu: null, memory: null, gpu: [], wifi: [], bluetooth: [] };
  ...
  if (result.code !== 0 || !result.stdout.trim()) return { available: false, temperature: null, battery: null, cpu: null, memory: null, gpu: [], wifi: [], bluetooth: [] };
  try { const parsed = JSON.parse(result.stdout.trim()); const battery = parsed.battery || null; const cpu = parsed.cpu || null; const memory = parsed.memory || null; const temperature = parsed.temperature || null; return { available: true, temperature: temperature?.CurrentTemperature ? { mainCelsius: Number(temperature.CurrentTemperature) / 10 - 273.15 } : null, battery: battery ? { available: true, percent: Number(battery.EstimatedChargeRemaining ?? -1), status: Number(battery.BatteryStatus ?? 0), voltage: Number(battery.Voltage || battery.DesignVoltage || 0) / 1000, currentCapacity: Number(battery.FullChargeCapacity || 0), designedCapacity: Number(battery.DesignCapacity || 0), currentMilliAmps: Number(battery.Current || 0), charging: [6, 7, 8, 9].includes(Number(battery.BatteryStatus)), acConnected: [2, 6, 7, 8, 9].includes(Number(battery.BatteryStatus)) } : null, cpu: cpu ? { model: cpu.Name, usagePercent: Number(cpu.LoadPercentage ?? -1), cores: Number(cpu.NumberOfCores ?? 0), speedGHz: Number(cpu.MaxClockSpeed || 0) / 1000 } : null, memory: memory ? { totalBytes: Number(memory.TotalVisibleMemorySize || 0) * 1024, availableBytes: Number(memory.FreePhysicalMemory || 0) * 1024 } : null, gpu: Array.isArray(parsed.gpu) ? parsed.gpu.map((item: any) => ({ model: item.Name, vendor: item.AdapterCompatibility, vramBytes: Number(item.AdapterRAM || 0), refreshRate: item.CurrentRefreshRate, mode: item.VideoModeDescription })) : [], wifi: Array.isArray(parsed.wifi) ? parsed.wifi.map((item: any) => ({ iface: item.Name, description: item.InterfaceDescription, mac: item.MacAddress, operstate: item.Status, speed: item.LinkSpeed })) : [], bluetooth: Array.isArray(parsed.bluetooth) ? parsed.bluetooth.map((item: any) => ({ name: item.FriendlyName, id: item.InstanceId, status: item.Status, connected: item.Status === 'OK' })) : [] }; } catch { return { available: false, temperature: null, battery: null, cpu: null, memory: null, gpu: [], wifi: [], bluetooth: [] }; }
}
...
export async function getCoreDiagnostics() {
  const [loadValue, memValue, tempValue, disks, batteryValue, osValue, cpuValue, graphicsValue, wifiValue, bluetoothValue, nativeValue] = await Promise.all([
    ...
    siCall<any>('wifiNetworks', []),
    siCall<any>('bluetoothDevices', []),
    getWindowsNativeTelemetry()
  ]);
  const resolvedBattery: any = batteryValue.hasBattery ? batteryValue : nativeValue.battery || batteryValue;
  const resolvedCpu: any = cpuValue.brand || cpuValue.speed ? cpuValue : nativeValue.cpu || cpuValue;
  const resolvedMemory: any = memValue.total > 0 ? memValue : nativeValue.memory || { total: os.totalmem(), used: os.totalmem() - os.freemem(), available: os.freemem() };
  const resolvedGpu: any[] = Array.isArray(graphicsValue.controllers) && graphicsValue.controllers.length ? graphicsValue.controllers : nativeValue.gpu;
  const resolvedWifi: any[] = Array.isArray(wifiValue) && wifiValue.length ? wifiValue : nativeValue.wifi;
  const resolvedBluetooth: any[] = Array.isArray(bluetoothValue) && bluetoothValue.length ? bluetoothValue : nativeValue.bluetooth;
  ...
  return {
    ...
    gpu: (Array.isArray(resolvedGpu) ? resolvedGpu : []).map((item: any) => ({ model: item.model, vendor: item.vendor, vramBytes: item.vram ?? item.vramBytes, utilizationPercent: item.utilizationGpu, refreshRate: item.refreshRate, mode: item.mode })),
    wifi: (Array.isArray(resolvedWifi) ? resolvedWifi : []).map((item: any) => ({ ssid: item.ssid, bssid: item.bssid, channel: item.channel, signalLevel: item.signalLevel, frequency: item.frequency, security: item.security, iface: item.iface, description: item.description, mac: item.mac, operstate: item.operstate, speed: item.speed })),
    bluetooth: (Array.isArray(resolvedBluetooth) ? resolvedBluetooth : []).map((item: any) => ({ name: item.name, mac: item.mac, connected: item.connected ?? item.status === 'OK', batteryLevel: item.batteryLevel, id: item.id })),
    storage: (Array.isArray(disks) ? disks : []).map((item: any) => ({ mount: item.mount, sizeBytes: item.size, usedBytes: item.used, usagePercent: item.size ? Number(((item.used / item.size) * 100).toFixed(1)) : 0 }))
  };
}

export async function getNetworkStatus() {
  const [netsRaw, netStatsRaw, connsRaw, external, latency] = await Promise.all([
    siCall('networkInterfaces', [] as any[]), siCall('networkStats', [] as any[]), siCall('networkConnections', [] as any[]),
  ...
  const nets = Array.isArray(netsRaw) ? netsRaw : netsRaw ? [netsRaw] : []; const netStats = Array.isArray(netStatsRaw) ? netStatsRaw : netStatsRaw ? [netStatsRaw] : []; const conns = Array.isArray(connsRaw) ? connsRaw : connsRaw ? [connsRaw] : [];
  return { timestamp: new Date().toISOString(), dependency: { systeminformation: Boolean(systemInformation) }, hostname: os.hostname(), latencyMs: typeof latency === 'number' && latency >= 0 ? latency : null, interfaces: nets.map((item: any) => ({ iface: item.iface, type: item.type, ip4: item.ip4, ip6: item.ip6, mac: item.mac, speedMbps: item.speed, dhcp: item.dhcp, internal: item.internal, operstate: item.operstate })), throughput: netStats.map((item: any) => ({ iface: item.iface, rxBytes: item.rx_bytes, txBytes: item.tx_bytes, rxSec: item.rx_sec, txSec: item.tx_sec })), connections: conns.slice(0, 80).map((item: any) => ({ protocol: item.protocol, localAddress: item.localAddress, localPort: item.localPort, peerAddress: item.peerAddress, peerPort: item.peerPort, state: item.state })), publicIp: external.code === 0 ? external.stdout.trim() : null };
}
...
function parseArgs(input: string) { return input.match(/(?:[^\s"']+|"[^"]*"|'[^']*')+/g)?.map((part) => part.replace(/^['"]|['"]$/g, '')) || []; }
export async function runAdb(commandLine: string) { const args = parseArgs(commandLine); if (!args.length) return { ok: false, stdout: '', stderr: 'Enter an ADB command, for example: devices -l', code: 2 }; const result = await exec('adb', args, 30000); return { ok: result.code === 0, stdout: result.stdout, stderr: result.stderr, code: result.code }; }
...
export async function currentLocation() {
  try { const response = await fetch('https://ipapi.co/json/', { headers: { 'User-Agent': 'JARVIS/1.0 (location service)' } }); if (!response.ok) throw new Error(`Location service returned ${response.status}`); const data = await response.json() as { latitude?: number; longitude?: number; city?: string; region?: string; country_name?: string; timezone?: string; ip?: string }; return { source: 'ip-geolocation', approximate: true, latitude: data.latitude ?? null, longitude: data.longitude ?? null, city: data.city ?? null, region: data.region ?? null, country: data.country_name ?? null, timezone: data.timezone ?? null, ip: data.ip ?? null }; } catch (error) { return { source: 'unavailable', approximate: true, error: error instanceof Error ? error.message : String(error), latitude: null, longitude: null }; }
}

export async function searchLocation(query: string) { const response = await fetch(`https://nominatim.openstreetmap.org/search?format=jsonv2&limit=8&addressdetails=1&q=${encodeURIComponent(query)}`, { headers: { 'User-Agent': 'JARVIS/1.0 location search' } }); if (!response.ok) throw new Error(`Map search returned ${response.status}`); return await response.json() as Array<{ display_name: string; lat: string; lon: string; type?: string; address?: Record<string, string> }>; }

export async function getWeather(latitude: number, longitude: number) { if (!Number.isFinite(latitude) || !Number.isFinite(longitude)) return { available: false, error: 'WEATHER_COORDINATES_UNAVAILABLE' }; const url = `https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&current=temperature_2m,relative_humidity_2m,apparent_temperature,is_day,precipitation,weather_code,cloud_cover,pressure_msl,wind_speed_10m,wind_direction_10m&timezone=auto`; try { const response = await fetch(url, { headers: { 'User-Agent': 'JARVIS/1.0 weather panel' } }); if (!response.ok) throw new Error(`Weather service returned ${response.status}`); const data = await response.json() as any; return { available: true, source: 'Open-Meteo', latitude, longitude, timezone: data.timezone, timestamp: data.current?.time, current: data.current || {} }; } catch (error) { return { available: false, source: 'Open-Meteo', error: error instanceof Error ? error.message : String(error) }; } }

export async function getSystemLogs() { if (process.platform === 'win32') { const result = await exec('powershell.exe', ['-NoProfile', '-Command', "Get-WinEvent -LogName Application -MaxEvents 30 | ForEach-Object { '{0} // {1} // {2}' -f $_.TimeCreated.ToString('s'), $_.LevelDisplayName, ($_.ProviderName + ': ' + $_.Message -replace '[\\r\\n]+',' ') }"] , 12000); return { source: 'Windows Application Event Log', lines: (result.stdout || result.stderr).split(/\r?\n/).filter(Boolean) }; } return { source: 'JARVIS local runtime', lines: [`${new Date().toISOString()} // Development host: ${os.platform()} ${os.release()}`, `${new Date().toISOString()} // Windows Event Log unavailable on this host`] }; }
