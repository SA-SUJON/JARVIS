...
async function syncPythonEnv(settings) {
...
function rejectPythonPending(error) {
...
function ensurePythonBridge() {
    ...
    if (!existsSync(script))
        throw new Error(`Python capability bridge not found at ${script}`);
    ...
    pythonBridge.stderr.on('data', (chunk) => {
        ...
        if (detail)
            mainWindow?.webContents.send('python:error', detail);
    });
...
function rawPythonRequest(op, payload, timeoutMs = 120000) {
...
function deterministicDecisions(query) {
    const clean = query.trim();
    const match = clean.match(/^generate\s+(?:an?\s+)?image\s+(?:of\s+)?(.+)$/i);
    if (match)
        return [`generate image ${match[1]}`];
    if (/^(open|close|play|content|google search|youtube search|system)\s+/i.test(clean))
        return [clean];
    return [];
}
...
async function writeSettings(input) {
    ...
    const safeProviders = providers.map(({ key, keys, ...p }) => {
        ...
        if (encrypted)
            encryptedKeys[p.id] = encrypted;
        return p;
    });
    const encryptedGeminiKeys = geminiKeys.map((k) => encryptSecret(k)).filter((k) => Boolean(k));
    const legacyKeys = {};
...
app.whenReady().then(() => {
        ...
        if (executable.length) {
            try {
                ...
                return {
                    ...
                    attempts: results.filter((item) => !item.ok).map((item) => item.detail || item.action),
                };
            }
            catch (error) {
                const message = error instanceof Error ? error.message : String(error);
    ...
    ipcMain.handle('stt:start', async (_event, language = 'en') => {
        ...
        voiceListener.once('close', (code, signal) => {
            ...
            voiceListener = null;
            sttStopping = false;
            if (!stoppedIntentionally && code !== 0)
