...
export async function webSearchAnswer(query) {
    ...
    try {
        ...
        const lines = [`According to live DuckDuckGo results for “${cleanQuery}”: `];
        if (instantJson.AbstractText)
            lines.push(`${instantJson.Heading ? `${instantJson.Heading}: ` : ''}${instantJson.AbstractText}`);
        hits.slice(0, 4).forEach((hit) => lines.push(`\n• ${hit.title}: ${hit.snippet || ''} (Source: ${hit.url})`));
        if (lines.length === 1)
            return { kind: 'search', answer: `I searched DuckDuckGo for “${cleanQuery}”, but no live indexed entries were returned. Please try rephrasing.` };
...
export function extractProviderKeys(provider) {
    const keys = [];
...
export async function listModels(provider) {
    ...
    const baseModels = curated.map((m) => ({
        id: m.id,
        label: m.label,
        ...
        if (provider.id === 'openai') {
            ...
            if (res.ok) {
                ...
                const fetched = (json.data ?? [])
                    ...
                    provider: provider.id,
                    tier: /mini/i.test(m.id) ? 'free' : 'paid',
                }));
                const map = new Map();
                baseModels.forEach((m) => map.set(m.id, m));
                fetched.forEach((m) => map.set(m.id, m));
        ...
        if (provider.id === 'anthropic') {
            ...
            if (res.ok) {
                ...
                const fetched = (json.data ?? []).map((m) => ({
                    ...
                    provider: provider.id,
                    tier: /haiku/i.test(m.id) ? 'free' : 'paid',
                }));
                const map = new Map();
                baseModels.forEach((m) => map.set(m.id, m));
                fetched.forEach((m) => map.set(m.id, m));
...
export async function utilityAnswer(query, identity) {
    ...
    if (/^(what(?:'s| is) |tell me )?(the )?time(?: in | at )?/i.test(text)) {
        ...
        try {
            ...
            if (res.ok) {
                const json = await res.json();
                return { kind: 'utility', answer: `The current time in ${json.timezone || tz} is ${new Date(json.datetime || Date.now()).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}.` };
            }
        }
    ...
    if (/^(calculate|compute|what is)\s+/i.test(text)) {
        ...
        if (result !== null)
            return { kind: 'utility', answer: `${expression} equals ${result}.` };
    }
    return null;
}
export async function failoverChat(providers, messages, preferred) {
    ...
    if (!ordered.length) {
        ...
        if (web)
            return { answer: web.answer, provider: 'web', attempts: ['No configured AI keys; switched to DuckDuckGo web lookup.'] };
        return {
    ...
    const web = await webSearchAnswer(lastUserQuery);
    if (web)
        return { answer: web.answer, provider: 'web', attempts: [...attempts, 'Switched to DuckDuckGo web search.'] };
    return {
