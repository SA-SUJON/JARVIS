...
export async function callPython(action, settings, payload = {}, timeout = 45000) {
    const status = pythonEngineStatus();
    if (!status.installed)
        throw new Error(`Python JARVIS engine is not present at ${status.root}`);
    return await new Promise((resolve, reject) => {
        ...
        const timer = setTimeout(() => { if (!settled) {
            settled = true;
            child.kill();
            reject(new Error(`Python engine timed out during ${action}`));
        } }, timeout);
        ...
        child.once('close', (code) => { if (settled)
            ...
            if (!parsed.ok && parsed.error)
                reject(new Error(`${parsed.error}${stderr ? ` — ${stderr.slice(0, 400)}` : ''}`));
            else if (code !== 0)
                reject(new Error(parsed.error || stderr || `Python engine exited with ${code}`));
            else
                resolve(parsed);
        }
        catch {
            reject(new Error(stderr || `Python engine returned invalid JSON: ${stdout.slice(0, 300)}`));
        } });
