...
function exec(command, args, timeout = 8000) {
    return new Promise((resolve) => {
        execFile(command, args, { timeout, windowsHide: true, maxBuffer: 1024 * 1024 }, (error, stdout, stderr) => resolve({
            stdout: String(stdout || ''),
            stderr: String(stderr || ''),
            code: error ? (typeof error.code === 'number' ? Number(error.code) : 1) : 0,
        }));
    });
}
...
let cachedPublicIp = { ip: null, collectedAt: 0 };
let cachedConnections = { list: [], collectedAt: 0 };
let cachedLocation = { data: null, collectedAt: 0 };
let cachedLogs = { lines: [], collectedAt: 0 };
...
export async function getCoreDiagnostics() {
    ...
    if (!cachedStaticHardware.storage || !cachedStaticHardware.storageCollectedAt || now - cachedStaticHardware.storageCollectedAt > 300000) {
        cachedStaticHardware.storage = await siCall('fsSize', []);
    ...
    const wifiNets = nets
        .filter((n) => /wi-?fi|wireless|wlan|802\.11/i.test(n.iface || n.type || ''))
        .map((item) => ({
    ...
    return {
        ...
        cpu: {
            usagePercent: Number(Number(Number.isFinite(loadValue.currentLoad) ? loadValue.currentLoad : 0).toFixed(1)),
        ...
        battery: {
            ...
            healthPercent: batteryValue.designedCapacity > 0 && batteryValue.currentCapacity > 0
...
export async function getNetworkStatus() {
    ...
    if (!cachedConnections.list.length || now - cachedConnections.collectedAt > 60000) {
        siCall('networkConnections', []).then((conns) => {
    ...
    return {
        ...
        connections: (cachedConnections.list || []).map((item) => ({
...
export async function runAdb(commandLine) {
    const args = parseArgs(commandLine);
    if (!args.length)
        return { ok: false, stdout: '', stderr: 'Enter an ADB command, for example: devices -l', code: 2 };
...
export async function searchLocation(query) {
    const response = await fetch(`https://nominatim.openstreetmap.org/search?format=jsonv2&limit=8&addressdetails=1&q=${encodeURIComponent(query)}`, { headers: { 'User-Agent': 'JARVIS/1.0 location search' }, signal: AbortSignal.timeout(6000) });
    if (!response.ok)
        throw new Error(`Map search returned ${response.status}`);
    return (await response.json());
}
export async function getWeather(latitude, longitude) {
    if (!Number.isFinite(latitude) || !Number.isFinite(longitude))
        return { available: false, error: 'WEATHER_COORDINATES_UNAVAILABLE' };
